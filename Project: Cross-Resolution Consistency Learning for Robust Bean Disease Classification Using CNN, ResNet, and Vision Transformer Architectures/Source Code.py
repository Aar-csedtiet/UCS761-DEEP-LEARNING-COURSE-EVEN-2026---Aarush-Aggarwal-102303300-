...
def main():
    ...
    run_unseen_evaluation(trained_models_by_model)
    
# ─────────────────────────────────────────────────────────────────────────────
# 14. UNSEEN DATASET EVALUATION  (2-class: Angular Leaf Spot | No Disease)
# ─────────────────────────────────────────────────────────────────────────────

UNSEEN_DATA_DIR    = "Bean Disease"   # ← change to your folder path
UNSEEN_CLASS_NAMES = ["Angular-Leaf-Spot", "No Disease Bean"]

def get_native_resolution(data_dir: str) -> int:
    """
    Peek at the first image in data_dir (recursively) and return its
    shorter spatial dimension, rounded to the nearest SCALES value.
    Falls back to 224 if nothing is found.
    """
    from PIL import Image as PILImage
    for root, _, files in os.walk(data_dir):
        for fname in files:
            if fname.lower().endswith((".jpg", ".jpeg", ".png", ".bmp")):
                img = PILImage.open(os.path.join(root, fname))
                native = min(img.size)           # shorter side
                # snap to the nearest scale in SCALES
                closest = min(SCALES, key=lambda s: abs(s - native))
                print(f"    Detected native resolution: {img.size} "
                      f"→ using scale {closest}")
                return closest
    print("    WARNING: No images found – defaulting to scale 224")
    return 224


def load_unseen_dataset(scale: int):
    """
    Load the unseen dataset at exactly the detected native scale.
    ...
    dataset = ImageFolder(UNSEEN_DATA_DIR, transform=tf)
    loader  = DataLoader(dataset, batch_size=BATCH_SIZE,
                         shuffle=False, num_workers=2, pin_memory=True)
    print(f"    Unseen dataset: {len(dataset)} images  |  "
          f"classes detected: {dataset.classes}")
    return loader, dataset.classes


def remap_logits_to_2class(logits: torch.Tensor,
                            original_classes: list,
                            unseen_classes: list) -> torch.Tensor:
    """
    The original model has 3 output logits:
        0 → Angular Leaf Spot
        1 → Bean Rust
        2 → Healthy

    The unseen dataset has 2 classes:
        Angular Leaf Spot  → keep as-is (original index 0)
        No Disease Bean    → treated as "Healthy" (original index 2)

    We extract only columns [0, 2] so the remapped 2-class logits align with
    the unseen dataset's label ordering.

    If the unseen folder uses a *different* ordering (e.g. class 0 = No Disease),
    adjust `col_map` below accordingly.
    """
    # Map each unseen class name to an original-model output column
    name_to_original_col = {
        name.lower().replace(" ", "_"): idx
        for idx, name in enumerate(original_classes)
    }

    # Heuristic remapping for the known unseen classes
    col_map = []
    for uc in unseen_classes:
        uc_key = uc.lower().replace(" ", "_").replace("-", "_")
        if "angular" in uc_key:
            col_map.append(0)          # Angular Leaf Spot → col 0
        elif "no_disease" in uc_key or "healthy" in uc_key or "no disease" in uc_key.replace("_", " "):
            col_map.append(2)          # No Disease / Healthy → col 2
        else:
            # fallback: find closest by name
            best = min(range(len(original_classes)),
                       key=lambda i: original_classes[i].lower() != uc.lower())
            col_map.append(best)
            print(f"    WARNING: Could not map '{uc}' – defaulting to col {best}")

    return logits[:, col_map]          # (B, 2)


def evaluate_unseen(model: nn.Module,
                    loader,
                    unseen_class_names: list,
                    original_class_names: list = CLASS_NAMES):
    """
    Run inference on the unseen loader.
    Remaps 3-class logits to 2 classes before computing argmax.
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            # Remap to 2-class space
            logits_2 = remap_logits_to_2class(
                logits, original_class_names, unseen_class_names
            )                                              # (B, 2)

            probs = torch.softmax(logits_2, dim=1).cpu().numpy()
            preds = logits_2.argmax(1).cpu().numpy()
...
def save_unseen_confusion_matrix(labels, preds,
                                  model_name: str,
                                  class_names: list):
    cm = confusion_matrix(labels, preds)
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Purples",
                xticklabels=class_names, yticklabels=class_names, ax=ax)
    ax.set_title(f"{model_name} — Unseen Dataset Confusion Matrix")
    ax.set_xlabel("Predicted"); ax.set_ylabel("True")
    plt.tight_layout()
    fname = os.path.join(RESULTS_DIR, f"{model_name}_unseen_confusion_matrix.png")
    plt.savefig(fname, dpi=150); plt.close()
    print(f"    Saved: {fname}")


def run_unseen_evaluation(trained_models_by_model: dict):
    """
    Entry point for unseen-dataset evaluation.
    Picks the best-scale model for each architecture (highest scale = richest
    features) and evaluates it on the unseen dataset at its native resolution.
    ...
    # Detect native resolution once
    native_scale = get_native_resolution(UNSEEN_DATA_DIR)
    unseen_loader, detected_classes = load_unseen_dataset(native_scale)

    # If the folder class names differ from UNSEEN_CLASS_NAMES, use detected ones
    class_names = (detected_classes
                   if len(detected_classes) == 2
                   else UNSEEN_CLASS_NAMES)

    unseen_report_lines = []

    for model_name, models_by_scale in trained_models_by_model.items():
        print(f"\n  ── {model_name}")

        # Use the largest trained scale (most informative)
        best_scale = max(models_by_scale.keys())
        model      = models_by_scale[best_scale]
        print(f"    Using weights from scale={best_scale}")

        labels, preds, probs = evaluate_unseen(model, unseen_loader,
                                               class_names)

        acc = (labels == preds).mean()
        print(f"    Unseen test accuracy : {acc:.4f}")

        # Confusion matrix plot
        save_unseen_confusion_matrix(labels, preds, model_name, class_names)

        # Text classification report
        cr = classification_report(labels, preds, target_names=class_names)
        print(f"\n{cr}")
        unseen_report_lines.append(
            f"=== {model_name} | unseen dataset | scale={best_scale} ===\n{cr}"
        )

    # Append unseen results to the existing text report
    report_path = os.path.join(RESULTS_DIR, "full_classification_report.txt")
    with open(report_path, "a") as f:
        f.write("\n" + "=" * 70 + "\n")
        f.write("UNSEEN DATASET EVALUATION RESULTS\n")
        f.write("=" * 70 + "\n\n")
        for line in unseen_report_lines:
            f.write(line + "\n")
    print(f"\n    Unseen results appended to: {report_path}")
